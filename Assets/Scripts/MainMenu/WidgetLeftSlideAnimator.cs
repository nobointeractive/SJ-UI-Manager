...
public class WidgetLeftSlideAnimator : UIAnimator
{
    ...
    public override float Show(UIAnimatable animatable)
    {
        ...
        foreach (var target in animatable.AnimatableTargets)
        {
            ...
            target.GetComponent<RectTransform>().anchoredPosition = new Vector2(-Screen.width / 2 - targetSizeX, 0);
    ...
    public override float Hide(UIAnimatable animatable)
    {
        ...
        foreach (var target in animatable.AnimatableTargets)
        {            
            target.GetComponent<RectTransform>().DOAnchorPosX(-Screen.width / 2 - targetSizeX, 0.3f).SetEase(Ease.InBack);
        }
        return 0.3f;
    }
}
